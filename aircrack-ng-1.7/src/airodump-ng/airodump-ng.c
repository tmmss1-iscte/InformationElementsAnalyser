...
static const char * OUI_PATHS[]
	= {"./airodump-ng-oui.txt",
	   "/home/kali/Desktop/wireshark-oui-list.txt",	
						   ...
						   sqlite3 *db)
{
	...
	switch (h80211[1] & IEEE80211_FC1_DIR_MASK)
	{
		...
		case IEEE80211_FC1_DIR_FROMDS:
			...
			/* FromDS packet, reject broadcast MACs (Nota: Nao percebi esta comparacao! Deixei comentada.)*/
	...
	int power;
	//power=st_cur->power;  //Antes estava assim
	power=ri->ri_power;
	int best_power ;
...
skip_station:
	...
	if (h80211[0] == IEEE80211_FC0_SUBTYPE_PROBE_REQ && st_cur != NULL)  
	{
		...
		// Verificar endereco MAC real ou aleatorio

		bool is_mac_addr_random;
		char mac_addr_type[10];
		...
		// Endereco MAC aleatorio
		if(is_mac_addr_random == true){

			// Geracao da footprint

			p = h80211 + 24;


			int ssid_flag = 0;
			int supported_rates_flag = 0;
			int extended_supported_rates_flag = 0;
			int DS_param_flag = 0;
			int HT_capabilities_flag = 0;
			int extended_capabilities_flag = 0;
			int VHT_capabilities_flag = 0;
			int RM_enabled_cap_flag = 0;
			int interworking_flag = 0;
			int supp_operating_classes_flag = 0;
			int vendor_specific_flag = 0;
			int vendor_1_specific_flag = 0;
			int vendor_2_specific_flag = 0;
			int vendor_3_specific_flag = 0;
			int vendor_4_specific_flag = 0;
			...
			uint64_t u64t;
			uint64_t u64t_supp_rates;
			uint64_t u64t_ext_supp_rates;
			uint64_t u64t_DS_Param;
			uint64_t u64t_HT_capabilities;
			uint64_t u64t_ext_capabilities;
			uint64_t u64t_VHT_capabilities;
			uint64_t u64t_RM_enabled_capabilities;
			uint64_t u64t_interworking;
			uint64_t u64t_supp_operating_classes;
			uint64_t u64t_vendor_specific_1;
			uint64_t u64t_vendor_specific_2;
			uint64_t u64t_vendor_specific_3;
			uint64_t u64t_vendor_specific_4;

			
			int vendor_specific_count = 0;

			char footprint[17];
			char supp_rates[17];
			char ext_supp_rates[17];
			char DS_param[17];
			char HT_cap[17];
			char ext_cap[17];
			char VHT_cap[17];
			char RM_enabled_cap[17];
			char interworking[17];
			char supp_operating_classes[17];
			char vendor_1[17];
			char vendor_2[17];
			char vendor_3[17];
			char vendor_4[17];

			char IE_utilized[22] = {'\0'};

			//Chars
			char array_supp_rates[8];
			char array_ext_supp_rates[35];
			char array_ds_param[3];
			char array_HT_capabilities[26];
			char array_ext_capabilities[35];
			char array_VHT_capabilities[24];
			char array_RM_enabled_cap[15];
			char array_interworking[15];
			char array_supp_operating_classes[15];
			char array_vendor_specific_1[110];
			char array_vendor_specific_2[110];
			char array_vendor_specific_3[110];
			char array_vendor_specific_4[110];

			//Hexadecimal
			char data_supp_rates[17]= {'\0'};
			char data_ext_supp_rates[70]= {'\0'};
			char data_ds_param[3]= {'\0'};
			char data_HT_capabilities[53]= {'\0'};
			char data_ext_capabilities[70]= {'\0'};
			char data_VHT_capabilities[49]= {'\0'};
			char data_RM_enabled_cap[30]= {'\0'};
			char data_interworking[30]= {'\0'};
			char data_supp_operating_classes[30]= {'\0'};
			char data_vendor_specific_1[220]= {'\0'};
			char data_vendor_specific_2[220]= {'\0'};
			char data_vendor_specific_3[220]= {'\0'};
			char data_vendor_specific_4[220]= {'\0'};
				...
				//{
					...
					case 0:
							...
							&& (p[1] > 1 || p[2] != ' '))
						{
							...
							for (i = 0; i < n; i++)
								if (p[2 + i] > 0 && p[2 + i] < ' ') goto skip_probe; // Analise do probe request saltava fora aqui!!

							/* got a valid ASCII probed ESSID, check if it's
							already in the ring buffer // Colocar aqui

							for (i = 0; i < NB_PRB; i++)
								if (memcmp(st_cur->probes[i], p + 2, n) == 0)
									goto skip_probe; // Analise do probe request saltava fora aqui!!
					...
					*/
					...
					//case 1:
					if(p[0] == 0x01){
						supported_rates_flag = 1;

						supp_rates_length = p[1];

						//unsigned char array_supp_rates[supp_rates_length];
						...
						//printf("[Supported Rates Length: %i ]\n", supp_rates_length);
						...
						for(i = 0; i < supp_rates_length; i++)
						{
							...
							sprintf(data_supp_rates+i*2, "%02X", p[2 + i]);
							array_supp_rates[i] = p[2 + i];
							array_v[pos] = p[2 + i];
							pos++;
							...
							printf("Supported Rates: %i \n" , rate);
							*/
						}


						u64t_supp_rates = t1ha0(array_supp_rates, sizeof(array_supp_rates), (uint64_t)seed);
						//printf("Supported Rates: t1ha0 64+Seed\t\t\tHex: %llX\n", u64t_supp_rates);
						snprintf(supp_rates, sizeof(supp_rates), "%llX", u64t_supp_rates);
						//break;
					}
					...
					//case 50:
					if(p[0] == 0x032){
						extended_supported_rates_flag = 1;

						extended_supp_rates_length = p[1];

						//unsigned char array_ext_supp_rates[extended_supp_rates_length];
						...
						//printf("[Extended Support Rates Length: %i ]\n", extended_supp_rates_length);

						//printf("Extended Supported Rates: [Mbits/sec]\n");
						...
						for(i = 0; i < extended_supp_rates_length; i++)
						{
							...
							sprintf(data_ext_supp_rates+i*2, "%02X", p[2 + i]);
							array_ext_supp_rates[i] = p[2 + i];
							array_v[pos] = p[2 + i];
							pos++;

							/*
							ext_rate = (int)p[2 + i];
							if(ext_rate > 128) ext_rate -= 128;
							ext_rate = (ext_rate*5)/10;
							
							printf("Extended Supported Rates: %i \n" , ext_rate);
							*/
						}


						//printf("Data_ext_supp_rates: %s \n", data_ext_supp_rates);


						u64t_ext_supp_rates = t1ha0(array_ext_supp_rates, sizeof(array_ext_supp_rates), (uint64_t)seed);
						//printf("Extended Supported Rates: t1ha0 64+Seed\t\t\tHex: %llX\n", u64t_ext_supp_rates);
						snprintf(ext_supp_rates, sizeof(ext_supp_rates), "%llX", u64t_ext_supp_rates);
						//break;
					}
					...
					if(p[0] == 0x03){
						...
						//unsigned char array_ds_param[DS_param_length];

						sprintf(data_ds_param, "%02X", p[0]);
						array_ds_param[0]=p[0];
						array_v[pos] = p[0];

						pos++;

						//printf("[DS Parameter Set Length: %i ]\n", DS_param_length);

						//printf("DS Parameter Set: %i \n", p[2]);

						st_cur->channel = p[2];

						u64t_DS_Param = t1ha0(array_ds_param, sizeof(array_ds_param), (uint64_t)seed);
						//printf("DS Parameter Set: t1ha0 64+Seed\t\t\tHex: %llX\n", u64t_DS_Param);
						snprintf(DS_param, sizeof(DS_param), "%llX", u64t_DS_Param);
						//break;


						//printf("Data_ds_param: %s. \n", data_ds_param);
					}
					

					//case 45:
					if(p[0] == 0x2D){
						HT_capabilities_flag = 1;

						HT_capabilities_length = p[1];

						//unsigned char array_HT_capabilities[HT_capabilities_length];
						...
						//printf("[HT Capabilities Length: %i ]\n", HT_capabilities_length);
						...
						for(i = 0; i < HT_capabilities_length; i++)
						{
							...
							/*
							if( i == 0 ){
								//ignorar bits do HT Support channel width do "HT Capabilities Info"
								array_HT_capabilities[i] = (p[2 + i] | 0x02);
								array_v[pos] = (p[2 + i] | 0x02);
							}else if( i == 1 ){
								//ignorar bits do MAX A-MSDU length e DSSS/CCK mode in 40 MHz do "HT Capabilities Info"
								array_HT_capabilities[i] = (p[2 + i] | 0x18);
								array_v[pos] = (p[2 + i] | 0x18);
							}else if( i > 2 && i < 13){
								//ignorar 10 bytes do "Rx Modulation and Coding Scheme (One bit per modulation)"
								array_HT_capabilities[i] = '0';
								array_v[pos] = '0';
							}else{
								array_HT_capabilities[i] = p[2 + i];
								array_v[pos] = p[2 + i];
							}
							*/
							...
							/*
							sprintf(data_HT_capabilities+i*2, "%02X", p[2 + i]);
							array_HT_capabilities[i] = p[2 + i];
							array_v[pos] = p[2 + i];
							*/
							...
							//if(i == HT_capabilities_length -1){ printf("\n");}
						}

						//printf("Data_HT_capabilities: %s \n", data_HT_capabilities);

						u64t_HT_capabilities = t1ha0(array_HT_capabilities, sizeof(array_HT_capabilities), (uint64_t)seed);
						//printf("HT Capabilities: t1ha0 64+Seed\t\t\tHex: %llX\n", u64t_HT_capabilities);
						snprintf(HT_cap, sizeof(HT_cap), "%llX", u64t_HT_capabilities);
						//break;
					}
					...
					//case 127:
					if(p[0] == 0x7F){
						extended_capabilities_flag = 1;

						extended_cap_length = p[1];

						//unsigned char array_ext_capabilities[extended_cap_length];
						//unsigned char array_ext_capabilities[1];
						...
						array_ext_capabilities[0] = p[0];
						array_ext_capabilities[1] = p[1];

						//printf("[Extended Capabilities Length: %i ]\n", extended_cap_length);
						...
						for(i = 0; i < extended_cap_length; i++)
						{
							...
							sprintf(data_ext_capabilities+i*2, "%02X", p[2 + i]);
							array_ext_capabilities[i] = p[2 + i];
							array_v[pos] = p[2 + i];
							pos++;

							//printf("%X ", p[2 + i]);

							//if(i == extended_cap_length -1){ printf("\n");}
						}

						//printf("Data_ext_capabilities: %s \n", data_ext_capabilities);
						

						u64t_ext_capabilities = t1ha0(array_ext_capabilities, sizeof(array_ext_capabilities), (uint64_t)seed);
						//printf("Extended Capabilities: t1ha0 64+Seed\t\t\tHex: %llX\n", u64t_ext_capabilities);
						snprintf(ext_cap, sizeof(ext_cap), "%llX", u64t_ext_capabilities);
						//break;
					}
					...
					//case 191:
					if(p[0] == 0xBF){
						VHT_capabilities_flag = 1;

						VHT_capabilities_length = p[1];

						//unsigned char array_VHT_capabilities[VHT_capabilities_length];
						...
						//printf("[VHT Capabilities Length: %i ]\n", VHT_capabilities_length);
						...
						for(i = 0; i < VHT_capabilities_length; i++)
						{
							/*
							if(i == 2){
								//ignorar "Number of Sounding Dimensions" bit - VHT Capabilities info
								array_VHT_capabilities[i] = p[2 + i] | 0x01;
								array_v[pos] = p[2 + i] | 0x01;

							}else{
								array_VHT_capabilities[i] = p[2 + i];
								array_v[pos] = p[2 + i];
								
							}
							*/
							sprintf(data_VHT_capabilities+i*2, "%02X", p[2 + i]);
							array_VHT_capabilities[i] = p[2 + i];
							array_v[pos] = p[2 + i];
							...
							//if(i == VHT_capabilities_length -1){ printf("\n");}
						}

						//printf("Data_VHT_capabilities: %s \n", data_VHT_capabilities);
						

						u64t_VHT_capabilities = t1ha0(array_VHT_capabilities, sizeof(array_VHT_capabilities), (uint64_t)seed);
						//printf("HT Capabilities: t1ha0 64+Seed\t\t\tHex: %llX\n", u64t_VHT_capabilities);
						snprintf(VHT_cap, sizeof(VHT_cap), "%llX", u64t_VHT_capabilities);
						//break;
					}
					...
					//case 70:
					if(p[0] == 0x46){
						RM_enabled_cap_flag = 1;

						RM_enabled_cap_length = p[1];

						//unsigned char array_RM_enabled_cap[RM_enabled_cap_length];
						...
						//printf("[RM Enabled Capabilities Length: %i ]\n", RM_enabled_cap_length);

						//printf("RM Enabled Capabilities: \n");

						for(i = 0; i < RM_enabled_cap_length; i++)
						{
							
							sprintf(data_RM_enabled_cap+i*2, "%02X", p[2 + i]);
							array_RM_enabled_cap[i] = p[2 + i];
							array_v[pos] = p[2 + i];
							...
							//if(i == RM_enabled_cap_length -1){ printf("\n");}
						}

						u64t_RM_enabled_capabilities = t1ha0(array_RM_enabled_cap, sizeof(array_RM_enabled_cap), (uint64_t)seed);
						//printf("RM Enabled Capabilities: t1ha0 64+Seed\t\t\tHex: %llX\n", u64t_RM_enabled_capabilities);
						snprintf(RM_enabled_cap, sizeof(RM_enabled_cap), "%llX", u64t_RM_enabled_capabilities);
						//break;
					}
					...
					if(p[0] == 0x6B){
						...
						//unsigned char array_interworking[interworking_length];
						...
						for(i = 0; i < interworking_length; i++)
						{
							...
							sprintf(data_interworking+i*2, "%02X", p[2 + i]);
							array_interworking[i] = p[2 + i];
							array_v[pos] = p[2 + i];
							...
							//if(i == interworking_length -1){ printf("\n");}
						}

						u64t_interworking = t1ha0(array_interworking, sizeof(array_interworking), (uint64_t)seed);
						//printf("Interworking: t1ha0 64+Seed\t\t\tHex: %llX\n", u64t_interworking);
						snprintf(interworking, sizeof(interworking), "%llX", u64t_interworking);
						//break;
					}
					...
					//case 59:
					if(p[0] == 0x3B){
						supp_operating_classes_flag = 1;

						supp_operating_classes_length = p[1];

						//unsigned char array_supp_operating_classes[supp_operating_classes_length];
						...
						//printf("[Supported Operating Classes Length: %i ]\n", supp_operating_classes_length);
						...
						for(i = 0; i < supp_operating_classes_length; i++)
						{
							...
							sprintf(data_supp_operating_classes+i*2, "%02X", p[2 + i]);
							array_supp_operating_classes[i] = p[2 + i];
							array_v[pos] = p[2 + i];
							...
							//if(i == supp_operating_classes_length -1){ printf("\n");}
						}

						u64t_supp_operating_classes = t1ha0(array_supp_operating_classes, sizeof(array_supp_operating_classes), (uint64_t)seed);
						//printf("Supported Operating Classes: t1ha0 64+Seed\t\t\tHex: %llX\n", u64t_supp_operating_classes);
						snprintf(supp_operating_classes, sizeof(supp_operating_classes), "%llX", u64t_supp_operating_classes);
						//break;
					}
					...
					if(p[0] == 0xDD){
						...
						//printf("[Vendor Specific Length: %i ]\n", vendor_specific_length);

						//printf("Vendor Specific %i: \n", vendor_specific_count+1);

						for(i = 0; i < vendor_specific_length; i++)
						{
							switch ( vendor_specific_count ){
								case 0:
									if (i == 5){
										array_vendor_specific_1[i] = '0';
									}else{
										array_vendor_specific_1[i] = p[2 + i];
									}
									break;
								case 1:
									if (i == 5){
										array_vendor_specific_2[i] = '0';
									}else{
										array_vendor_specific_2[i] = p[2 + i];
									}
									break;
								case 2:
									if (i == 5){
										array_vendor_specific_3[i] = '0';
									}else{
										array_vendor_specific_3[i] = p[2 + i];
									}
									break;
								case 3:
									if (i == 5){
										array_vendor_specific_4[i] = '0';
									}else{
										array_vendor_specific_4[i] = p[2 + i];
									}
									break;
							}
							if (i == 5 || i == 7){
							...
							if(i == 3){
								...
								// Procura pelo campo UUID-E 

								//Microsoft Corp. (00:50:F2): WPS (0x04)
								if(p[2 + i - 3] == 0x00 && p[2 + i - 2] == 0x50 && p[2 + i - 1] == 0xF2 && p[2 + i] == 0x04){

									//printf("Microsoft Corp.: WPS!! \n");

									if(p[2 + i + 1] != '\0'){

										unsigned char *c = p + 6;

										//Iteracao sobre os Data Elements do campo WPS
										while( c < p + vendor_specific_length){

											printf("c[0] c[1] - Data Element: %X %X \n", c[0], c[1]);

											printf("c[2] c[3] - Data Element length: %X %X \n", c[2], c[3]);

											int data_element_length = c[2] | c[3] ;

											printf("data_element_length: %i \n", data_element_length);

											//Elemento UUID-E
											if( c[0] == 0x10 && c[1]  == 0x47){

												printf("ELEMENTO UUID-E!! \n");

												unsigned char array_UUID_E[data_element_length];

												printf("array_UUID_E: ");

												int a;

												for(a = 0; a < data_element_length; a++){

													array_UUID_E[a] = c[4 + a];

													printf("%X", c[4 + a]);

													if(a == data_element_length - 1){ printf("\n");}

												}

												//Gerar footprint apenas com os dados do UUID-E (?)
								...
								// Identificar dispositivos da Apple (?)
						...
						{
							vendor_1_specific_flag = 1;

							for(i = 0; i < vendor_specific_length; i++){
								sprintf(data_vendor_specific_1+i*2, "%02X", array_vendor_specific_1[i]);
							}

							//printf("Data_vendor_1: %s \n", data_vendor_specific_1);

							u64t_vendor_specific_1 = t1ha0(array_vendor_specific_1, sizeof(array_vendor_specific_1), (uint64_t)seed);
							//printf("Vendor Specific 1: t1ha0 64+Seed\t\t\tHex: %llX\n", u64t_vendor_specific_1);
							snprintf(vendor_1, sizeof(vendor_1), "%llX", u64t_vendor_specific_1);
						}else if(vendor_specific_count == 1)
						{
							vendor_2_specific_flag = 1;

							for(i = 0; i < vendor_specific_length; i++){
								sprintf(data_vendor_specific_2+i*2, "%02X", array_vendor_specific_2[i]);
							}

							//printf("Data_vendor_2: %s \n", data_vendor_specific_2);

							u64t_vendor_specific_2 = t1ha0(array_vendor_specific_2, sizeof(array_vendor_specific_2), (uint64_t)seed);
							//printf("Vendor Specific 2: t1ha0 64+Seed\t\t\tHex: %llX\n", u64t_vendor_specific_2);
							snprintf(vendor_2, sizeof(vendor_2), "%llX", u64t_vendor_specific_2);
						}else if(vendor_specific_count == 2)
						{
							vendor_3_specific_flag = 1;

							for(i = 0; i < vendor_specific_length; i++){
								sprintf(data_vendor_specific_3+i*2, "%02X", array_vendor_specific_3[i]);
							}

							//printf("Data_vendor_3: %s \n", data_vendor_specific_3);

							u64t_vendor_specific_3 = t1ha0(array_vendor_specific_3, sizeof(array_vendor_specific_3), (uint64_t)seed);
							//printf("Vendor Specific 3: t1ha0 64+Seed\t\t\tHex: %llX\n", u64t_vendor_specific_3);
							snprintf(vendor_3, sizeof(vendor_3), "%llX", u64t_vendor_specific_3);
						}else
						{
							vendor_4_specific_flag = 1;

							for(i = 0; i < vendor_specific_length; i++){
								sprintf(data_vendor_specific_4+i*2, "%02X", array_vendor_specific_4[i]);
							}

							//printf("Data_vendor_4: %s \n", data_vendor_specific_4);

							u64t_vendor_specific_4 = t1ha0(array_vendor_specific_4, sizeof(array_vendor_specific_4), (uint64_t)seed);
							//printf("Vendor Specific 4: t1ha0 64+Seed\t\t\tHex: %llX\n", u64t_vendor_specific_4);
							snprintf(vendor_4, sizeof(vendor_4), "%llX", u64t_vendor_specific_4);
						}
						vendor_specific_count++;
			...
			if(supported_rates_flag == 0){ strcpy(supp_rates, "NaN");}else{ strcat(IE_utilized, "1 ");}
			if(extended_supported_rates_flag == 0){ strcpy(ext_supp_rates, "NaN");}else{ strcat(IE_utilized, "50 ");}
			if(DS_param_flag == 0){ strcpy(DS_param, "NaN");}else{ strcat(IE_utilized, "3 ");}
			if(HT_capabilities_flag == 0){ strcpy(HT_cap, "NaN");}else{ strcat(IE_utilized, "45 ");}
			if(extended_capabilities_flag == 0){ strcpy(ext_cap, "NaN");}else{ strcat(IE_utilized, "127 ");}
			if(VHT_capabilities_flag == 0){ strcpy(VHT_cap, "NaN");}else{ strcat(IE_utilized, "191 ");}
			if(RM_enabled_cap_flag == 0){ strcpy(RM_enabled_cap, "NaN");}else{ strcat(IE_utilized, "70 ");}
			if(interworking_flag == 0){ strcpy(interworking, "NaN");}else{ strcat(IE_utilized, "107 ");}
			if(supp_operating_classes_flag == 0){ strcpy(supp_operating_classes, "NaN");}else{ strcat(IE_utilized, "59 ");}
			if(vendor_1_specific_flag == 0){ strcpy(vendor_1, "NaN");}else{ strcat(IE_utilized, "221(1) ");}
			if(vendor_2_specific_flag == 0){ strcpy(vendor_2, "NaN");}else{ strcat(IE_utilized, "221(2) ");}
			if(vendor_3_specific_flag == 0){ strcpy(vendor_3, "NaN");}else{ strcat(IE_utilized, "221(3) ");}
			if(vendor_4_specific_flag == 0){ strcpy(vendor_4, "NaN");}else{ strcat(IE_utilized, "221(4) ");}
			

			// seed = 0
			//u64t=t1ha0(new_array, sizeof(new_array), (uint64_t)0);
			...
			//Para debug
			printf("Probe Request | Source: %s | Footprint: %s | SEQ: %i \n", mac_address, footprint, seq);
			
			// Inserir linha na base de dados
			putInToInformationElementsDB(mac_address , footprint, IE_utilized, data_supp_rates, data_ext_supp_rates, data_ds_param, data_HT_capabilities, data_ext_capabilities, data_VHT_capabilities, data_RM_enabled_cap, data_interworking, data_supp_operating_classes, data_vendor_specific_1, data_vendor_specific_2, data_vendor_specific_3, data_vendor_specific_4, milliseconds/1000, milliseconds%1000, received_power, manuf, seq, db);
...
skip_probe:
	...
	if (h80211[0] == IEEE80211_FC0_SUBTYPE_BEACON
		/*|| h80211[0] == IEEE80211_FC0_SUBTYPE_PROBE_RESP*/)	//POR AGORA ESTAO APENAS A SEREM CAPTURADOS BEACONS!!
	{
...
void putInToInformationElementsDB(char* mac_address, char* footprint, char* IE_utilized, char* supp_rates, char* ext_supp_rates, char* DS_param, char* HT_cap, char* ext_cap, char* VHT_cap, char* RM_enabled_cap, char* interworking, char* supp_operating_classes, char* vendor_1, char* vendor_2, char* vendor_3, char* vendor_4, long long milliseconds_quocient, int milliseconds_remainder, char* received_power, char* manuf, int seq, sqlite3 *db){
    char *zErrMsg = 0;
    int result;
	...
	// tabela de information_elements

	char sql_insert[540];

	
	sprintf(sql_insert, "INSERT INTO Information_Elements values ( '%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s', strftime('%%Y-%%m-%%d %%H:%%M:%%f', %lld.%03d, 'unixepoch') ,'%s','%s','%i');", mac_address, footprint, IE_utilized, supp_rates, ext_supp_rates, DS_param, HT_cap, ext_cap, VHT_cap, RM_enabled_cap, interworking, supp_operating_classes, vendor_1, vendor_2, vendor_3, vendor_4, milliseconds_quocient, milliseconds_remainder, received_power, manuf, seq);

	//printf(sql_insert);

	result = sqlite3_exec(db, sql_insert, callback, 0, &zErrMsg);
	if (result != SQLITE_OK ) {
...
int main(int argc, char * argv[])
{
	...
	*/
    ...
    char *err_msg = 0;
	#ifdef HAVE_SQLITE
		int rc = sqlite3_open("/home/kali/Desktop/InformationElementsAnalyser/DB/InformationElements.db", &db);
